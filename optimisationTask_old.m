    ...
    methods
        ...
        function obj = optimisationTask(varargin) %constructor
            ...
            elseif isa(varargin{1}, 'char')
                ...
                obj.TimeIntervalOut = varargin{3};
                obj.ObjectiveParameter = varargin{4};
        ...
        function obj = startGeneticOptimisation(obj)
            obj.History.BestScoresInSelection = [];
            obj.History.BestIndividualsInSelection = [];
            obj.History.TestedGens = [];
            obj.History.TestedGensScores = [];
            ...
            function [state, options, changed] = getOutputArray(options,state,~)%Saves best results from each generation
                ...
                obj.History.TestedGens = uint8([obj.History.TestedGens; state.Population]);
                obj.History.TestedGensScores = int16([obj.History.TestedGensScores; state.Score]);
                ...
                obj.History.TestedGensScores = obj.History.TestedGensScores(Indexes);
        ...
        function score = gaSocket(obj, GaVector)
            try
                indexes = ismember(obj.History.TestedGens, GaVector, 'rows');
            catch
                indexes = 0;
            end
            if any(indexes)
                score = obj.History.TestedGensScores(indexes);
            else
